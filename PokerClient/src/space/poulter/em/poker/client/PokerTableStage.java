...
import space.poulter.em.poker.Cards;
import space.poulter.em.poker.Card;
import space.poulter.em.poker.CommandFormatException;
import space.poulter.em.poker.PlayerData;
import space.poulter.em.poker.Poker.PokerAction;
import space.poulter.em.poker.PokerTableData;
import space.poulter.em.poker.Util;
import space.poulter.em.poker.client.PokerClient.ClientSideSocket;
...
public class PokerTableStage extends Stage {
    ...
    private class TableSection {
        ...
        public TableSection(int index) {
             ...
             * button is clicked.
             */
            sr.btn.setOnMouseClicked((MouseEvent e) -> {
                sendCommand("sit:"+getSeatID());
            });
         ...
         * @param isSittable 
         */
        public void setIsFree(boolean isFree, boolean isSittable) {
            this.isFree = isFree;
        ...
        public void showCardFronts(Cards hand) {
    ...
    private class HandRegion extends HBox {
        ...
        public void showCardFronts(Cards cards) {
            /* Make sure that we do have 2 cards */
            if(cards.size()<1)
                cards.setCard(Card.EMPTY_CARD, 0);
            if(cards.size()<2) 
                cards.setCard(Card.EMPTY_CARD, 1);
    ...
    private class CardRegion extends StackPane {
         ...
         */
        public CardRegion(int width, int height) {
        ...
        public void showFront(Card c) {
            ...
            if(!c.equals(Card.EMPTY_CARD)) {
                ...
                switch(c.getSuit()) {
                    case 'S': t.setFill(Color.DARKBLUE);
                              break;
                    case 'H': t.setFill(Color.ORANGERED);
                              break;
                    case 'D': t.setFill(Color.DARKORANGE);
                              break;
                    case 'C': t.setFill(Color.DARKGREEN);
                              break;
                    default: t.setFill(Color.BLACK);
                }
            }
        }
    }
     ...
     */
    public PokerTableStage(PokerTableDataClient dat, ClientSideSocket sock) {
        socket = sock;
        data = dat;
        playerSeat = null;
        init();  
    ...
    private void initButtons() {
        ...
        foldBtn.setOnMouseClicked((MouseEvent e) -> {
            sendCommand("game:fold:"+playerSeat.getSeatID());
        });
        checkBtn.setOnMouseClicked((MouseEvent e) -> {
            sendCommand("game:check:"+playerSeat.getSeatID());
        });
        callBtn.setOnMouseClicked((MouseEvent e) -> {
            sendCommand("game:call:"+playerSeat.getSeatID());
        });
        raiseBtn.setOnMouseClicked((MouseEvent e) -> {
            sendCommand("game:raise:"+playerSeat.getSeatID()+":"+(int)raiseSlider.getValue());
        });
    ...
    private void initTimer() {
        ...
        /* Create the timer bar, and make it the corrent shape */
     ...
     * Util.expandCommand(). 
     */
    protected void processCommand(String com) throws CommandFormatException {
...
//</editor-fold>
     ...
     */
    protected void processGameCommand(List<String> commands) 
        ...
        */
        ...
        if(commands.get(0).equals("winner")) {
            ...
            switch(commands.size()) {
                 ...
                 * argument
                 */
                case 4: winningHand = commands.get(3);
                ...
                default: throw new CommandFormatException(data.getTableID(), true,
                        String.join(":", commands));
            }
            ...
            if(commands.get(2).equals("action")) {
                ...
                try {
                    data.playerOnSeat(seatIndex).setAction(
                            PokerAction.valueOf(commands.get(3)));
    ...
    private void takeSeat(Integer index) {
        ...
        seats.entrySet().forEach((entry) -> 
            /* Set all of the seats as not being sittable */
            entry.getValue().setIsFree(entry.getValue().isFree, false)
        );
     ...
     * 
     * @param o
     * @return true if the two tables have the same id, false otherwise
     */
    @Override
    public boolean equals(Object o) {
