 ...
 */
public class OldPokerTable {
    private PokerTableData dat;
    private List<PokerSocket> connectedSockets;
    ...
    void processCommand(String command, PokerSocket socket) {
    ...
    void processGameCommand(String command, PokerSocket socket) {
        ...
        if (substring.equals("fold:")) {
            ...
            if (dat.seatIsOccupied(index) && dat.playerOnSeat(index).getSocket().equals(socket) && dat.playerOnSeat(index).isInHand()) {
                dat.playerOnSeat(index).setAction(Util.PokerAction.FOLD);
            }
            return;
        }
        if (substring.equals("check:")) {
            ...
            if (dat.seatIsOccupied(index) && dat.playerOnSeat(index).getSocket().equals(socket) && dat.playerOnSeat(index).isInHand()) {
                dat.playerOnSeat(index).setAction(Util.PokerAction.CHECK);
            }
            return;
        }
        ...
        if (substring.equals("call:")) {
            ...
            if (dat.seatIsOccupied(index) && dat.playerOnSeat(index).getSocket().equals(socket) && dat.playerOnSeat(index).isInHand()) {
                ...
                dat.playerOnSeat(index).setAction(Util.PokerAction.CALL);
            }
            return;
        }
        ...
        if (substring.equals("raise:")) {
            ...
            if (dat.seatIsOccupied(index) && dat.playerOnSeat(index).getSocket().equals(socket) && dat.playerOnSeat(index).isInHand()) {
                dat.playerOnSeat(index).setAction(Util.PokerAction.RAISE);
    ...
    void sendCommand(String command, PokerSocket socket) {
        try {
    ...
    private int bettingRound(Integer i, Integer bet) throws IllegalMonitorStateException, InterruptedException {
        ...
        do {
            ...
            if (dat.playerOnSeat(i).getAction().equals(Util.PokerAction.NONE)) {
                dat.playerOnSeat(i).setAction(Util.PokerAction.FOLD);
                //TODO check if possible to do so
            }

            if (dat.playerOnSeat(i).getAction().equals(Util.PokerAction.CALL)) {
            ...
            } else if (dat.playerOnSeat(i).getAction().equals(Util.PokerAction.RAISE)) {
            ...
            if (dat.playerOnSeat(i).getAction().equals(Util.PokerAction.FOLD)) {
                if (dat.removePlayerFromHand(i) == 1) {
                    return 1;
                }
            }

            if (dat.playerOnSeat(i).getAction().equals(Util.PokerAction.RAISE)) {
                endIndex = i;
                do {
                    endIndex++;
            ...
            dat.playerOnSeat(i).setAction(Util.PokerAction.NONE);
        ...
        while (true) {
            ...
            try {
                ...
                for (Map.Entry<Integer, PlayerData> player : dat.getPlayerAndIndex().entrySet()) {
                    ...
                    for (PokerSocket sock : connectedSockets) {
                ...
                for (Map.Entry<Integer, PlayerData> player : dat.getPlayerAndIndex().entrySet()) {
                    ...
                    player.getValue().getHand().setCard(1, d.drawCard());
                    ...
                    player.getValue().setAction(Util.PokerAction.NONE);
                    //dat.playerOnSeat(hand.getKey()).setAction(Util.PokerAction.NONE);
                }
                //</editor-fold>
    ...
    public void addSocket(PokerSocket s) {
        connectedSockets.add(s);
    }

    public void removeSocket(PokerSocket s) {
        connectedSockets.remove(s);
    }

    public List<PokerSocket> getSockets() {
        return connectedSockets;
    }

    public boolean hasSocket(PokerSocket s) {
