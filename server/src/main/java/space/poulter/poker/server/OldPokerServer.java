 ...
 */
public class OldPokerServer extends Thread {
    ...
    Map<Integer, OldPokerTable> tables;
    BiMap<Integer, PokerSocket> sockets;
    ServerSocket serverSocket;

    private OldPokerServer(String[] args) {
    ...
    public static void main(String[] args) {
        OldPokerServer newServer = new OldPokerServer(args);
        newServer.init();
        newServer.start();
    }
    ...
    private void init() {
        ...
        for (int i = 0; i < numTables / 2; i++) {
            OldPokerTable newTable = new OldPokerTable();
            newTable.init(i + 1, 6);
            tables.put(i + 1, newTable);
            newTable = new OldPokerTable();
    ...
    public class ServerSideSocket extends PokerSocket {
...
//</editor-fold>
            ...
            if (str.equals("Get Tables")) {
                ...
                try {
                    write("Table List:");
                    //write(tables.size());
                    for (Map.Entry<Integer, OldPokerTable> table : tables.entrySet()) {
                        PokerTableData dat = table.getValue().getData();
                        //write(dat);
                    }
                } catch (IOException e) {
