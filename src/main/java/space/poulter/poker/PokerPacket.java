...
@Immutable
final public class PokerPacket {
    ...
    public static PokerPacket createPacket(@NotNull PacketCode code, @NotNull Object... objects) {
        ...
        /* If the code is NONE, we shouldn't process the packet. This should never occur. */
        if (code == PacketCode.NONE) {
            return new PokerPacket(new byte[0]);
        }

        /* Create the packet with the Code, and then add all other objects. */
        return new PokerPacket(new PokerPacket(new byte[]{code.getVal()}).addObjects(objects));
    }
    ...
    public String getString(int offset) {
        ...
        Integer strLen = getInteger(offset);
        if (strLen == null || strLen < 0) return "";
    ...
    public PokerPacket addAll(Object... objects) {
        return new PokerPacket(addObjects(objects));
    }

    private byte[] addObjects(@NotNull Object... objects) {
        ...
        for (Object o : objects) {
            if (o instanceof Integer) {
                ...
                byteList.addAll(Bytes.asList(strBytes));
                ...
                /* If we have an array of bytes, just copy them in. */
                byteList.addAll(Bytes.asList((byte[]) o));
            } else if (o instanceof PokerPacket) {
                byteList.addAll(Bytes.asList(((PokerPacket) o).bytes));
            } else if (o instanceof Byte[]) {
                Collections.addAll(byteList, (Byte[]) o);
            } else {
